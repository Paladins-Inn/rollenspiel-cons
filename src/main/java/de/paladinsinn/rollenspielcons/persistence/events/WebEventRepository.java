 ...
 */
@Repository
public interface WebEventRepository extends PagingAndSortingRepository<JpaWebEvent, Long> {
  @Query("SELECT e FROM WebEvent e WHERE e.temporalData.startDay >= :start ORDER BY e.temporalData.startDay ASC")
  List<JpaWebEvent> findByStartDay(@Param("start") @NotNull LocalDateTime start);
  
  @Query("SELECT e FROM WebEvent e WHERE e.owner.owner = :owner ORDER BY e.temporalData.startDay ASC")
  List<JpaWebEvent> findByOwner(@Param("owner") String owner);
}
