...
. The java library `de.paladins-inn.torganized-play:rollenspiel-cons-store` needs to be added to the dependency list of the client.
The `de.paladins-inn.torganized-play:rollenspiel-cons-client` will be automatically included.
...
together {
  ...
  component "rollenspiel-cons" as scsUser << External >>
}
...
It will provide a `Identity` as specified in the  `users.client` component.

== Use a locally defined data store

. The java library `de.paladins-inn.torganized-play:rollenspiel-cons-client` needs to be added to the dependency list of the client.
